import{s as w}from"./index-BtcxLgic.js";const A=async({to:t,subject:e,html:r,from:n})=>{try{console.log(`[Resend Client] Dispatching email to: ${t} via backend proxy...`);const i=await fetch("/api/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t,subject:e,html:r,from:n})});if(i.ok)return{success:!0,id:(await i.json()).id||"msg_"+Math.random().toString(36).substr(2,9)};const p=await i.text();console.warn(`[Resend Client] Backend endpoint failed (${i.status}): ${p}. Falling back...`)}catch(o){console.warn(`[Resend Client] Backend proxy unreachable: ${o.message}. Falling back...`)}const l="re_4aKppTUo_CJJ3a2FR74NuRQSh7XmTgy9c";try{console.log("[Resend Client] Direct dispatch attempt via client key...");const o=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${l}`,"Content-Type":"application/json"},body:JSON.stringify({from:"Sparkles Apartments <onboarding@resend.dev>",to:[t],subject:e,html:r})});if(o.ok)return{success:!0,id:(await o.json()).id};const i=await o.json();console.error("[Resend Client] Direct Resend API failed:",i)}catch(o){console.error("[Resend Client] Direct Resend API error:",o)}return console.warn(`[Resend Client] Simulating email delivery to: ${t}`),await new Promise(o=>setTimeout(o,800)),{success:!0,simulated:!0}},V=async(t,e)=>{if(!e)return console.warn(`[Automation Engine] Trigger aborted for ${t}: Missing payload.`),{success:!1,reason:"Missing booking payload"};try{console.log(`[Automation Engine] Triggered event: "${t}"`);const{data:r}=await w.from("system_settings").select("setting_key, setting_value").in("setting_key",["notification_engine_active","contact_logo","contact_address","contact_phone","contact_email","system_theme"]),n=r?.reduce((d,a)=>(d[a.setting_key]=a.setting_value,d),{})||{};if(!(n.notification_engine_active==="true"||n.notification_engine_active===!0||n.notification_engine_active===void 0))return console.log("[Automation Engine] Engine is toggled offline in System Control."),{success:!1,reason:"Notification engine inactive"};const o=n.system_theme||"theme-luxe-gold",p={"theme-slate-dark":"#64748B","theme-luxe-gold":"#DF6853","theme-emerald-green":"#10B981","theme-royal-blue":"#3B82F6","theme-sunset-orange":"#F97316","theme-rose-burgundy":"#F43F5E","theme-midnight-purple":"#A855F7","theme-ocean-teal":"#14B8A6"}[o]||"#DF6853",h=n.contact_logo||"https://images.unsplash.com/photo-1512918728675-ed5a9ecdebfd?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",T=n.contact_address||"Plot 572 Iduwa Ogenyi Street Mabushi, Off Ahmadu Bello Way, Abuja",M=n.contact_phone||"08033214684, 08062332639, 08171278657",R=n.contact_email||"info@sparklesapartments.ng",{data:u,error:y}=await w.from("automation_rules").select("*, notification_templates(*)").eq("trigger_event",t).eq("is_active",!0);if(y)return console.error("[Automation Engine] Failed to fetch active rules:",y),{success:!1,error:y.message};if(!u||u.length===0)return console.log(`[Automation Engine] Zero active rules configured for event "${t}".`),{success:!0,count:0};console.log(`[Automation Engine] Processing ${u.length} automations for "${t}"...`);const v=[];for(const d of u){const a=d.notification_templates;if(!a)continue;const g=e.profiles||{},$=e.guest_name||`${g.first_name||""} ${g.last_name||""}`.trim()||"Valued Guest",F=e.guest_email||e.email||g.email||"guest@example.com",z=e.guest_phone||e.phone||g.phone||"N/A",E=e.booking_reference||e.id||"BKG-MOCK",B=e.check_in_date||e.check_in||"N/A",L=e.check_out_date||e.check_out||"N/A",c=a.channel==="email"?F:z;if(!c||c==="N/A"){console.warn(`[Automation Engine] Skipping rule "${d.name}": No recipient detail.`);continue}const O=e.room_number||e.rooms&&e.rooms.room_number||"N/A",j=e.room_details||e.rooms&&e.rooms.name||"Premium Suite",P=e.total_amount||e.total_amount_ngn||e.total_price||"0.00",N=e.total_paid||e.amount_paid||e.amount_paid_ngn||"0.00",I=e.balance_due!==void 0?e.balance_due:(Number(P)-Number(N)).toFixed(2),G=e.payment_status||"Pending",K=e.payment_amount||e.amount||"0.00",J=e.payment_ref||e.payment_reference||"N/A",U=e.payment_method||"N/A",W=e.payment_date||new Date().toLocaleDateString(),Y=e.invoice_number||"INV-"+E,k=s=>s?s.replace(/{{guest_name}}/g,$).replace(/{{booking_ref}}/g,E).replace(/{{check_in}}/g,B).replace(/{{check_out}}/g,L).replace(/{{room_number}}/g,O).replace(/{{room_details}}/g,j).replace(/{{total_amount}}/g,Number(P).toLocaleString()).replace(/{{total_paid}}/g,Number(N).toLocaleString()).replace(/{{balance_due}}/g,Number(I).toLocaleString()).replace(/{{payment_status}}/g,G).replace(/{{payment_amount}}/g,Number(K).toLocaleString()).replace(/{{payment_ref}}/g,J).replace(/{{payment_method}}/g,U.toUpperCase()).replace(/{{payment_date}}/g,W).replace(/{{invoice_number}}/g,Y):"",C=k(a.subject||"Sparkles Apartments Update"),x=k(a.body||"");let m="failed",b=null,f=!1;if(a.channel==="email"){const s=`
        ...
        `,_=await A({to:c,subject:C,from:"booking@sparklesapartments.ng",html:s});if(_.success?(m="sent",f=!!_.simulated):b=_.error||"SMTP routing failure",c!=="booking@sparklesapartments.ng")try{console.log("[Automation Engine] Forwarding admin copy of booking update to booking@sparklesapartments.ng...");const S=`
              ...
              ${s}
            `;await A({to:"booking@sparklesapartments.ng",subject:`[ADMIN] ${C}`,from:"booking@sparklesapartments.ng",html:S})}catch(S){console.warn("[Automation Engine] Failed to dispatch admin copy:",S)}}else if(a.channel==="sms"){const s=await q({to:c,message:x});s.success?(m="sent",f=!!s.simulated):b=s.error||"SMS Gateway routing failure"}else console.log(`[Automation Engine] Simulating "${a.channel}" dispatch to ${c}:
${x}`),await new Promise(s=>setTimeout(s,400)),m="sent",f=!0;try{const{error:s}=await w.from("notification_logs").insert([{recipient:c,channel:a.channel,template_name:a.name,status:m,error_message:b,sent_at:new Date().toISOString()}]);s&&console.error("[Automation Engine] Log insertion error:",s)}catch(s){console.error("[Automation Engine] Log commit exception:",s)}v.push({ruleName:d.name,channel:a.channel,status:m,simulated:f})}return{success:!0,executions:v}}catch(r){return console.error("[Automation Engine] Core trigger execution crash:",r),{success:!1,error:r.message}}},q=async({to:t,message:e})=>{try{console.log(`[SMS Client] Dispatching SMS to: ${t} via backend proxy...`);const n=await fetch("/api/sms/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t,message:e})});if(n.ok){const o=await n.json();return{success:!0,id:o.messageId,simulated:!!o.simulated}}const l=await n.text();return console.warn(`[SMS Client] Backend SMS proxy failed: ${l}`),{success:!1,error:l}}catch(r){return console.error(`[SMS Client] Backend SMS proxy unreachable: ${r.message}`),{success:!1,error:r.message}}},Q=async({email:t,firstName:e,lastName:r,password:n=null})=>{const l=`${window.location.origin}/login`,o=n?"Your Sparkles Apartments Credentials & Account Details":"Welcome to Sparkles Apartments - Premium Luxury Shortlets",i=`
